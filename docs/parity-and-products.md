...
### GAPS / DECISIONS NEEDED (current has it, skyhook does not — and it isn't clearly "discontinued")
1. **`annotations/filtered_goa_uniprot_all.gaf.gz` (10.2 G) + `_noiea` variants** — the "all UniProt" mega-GAF. Not produced by the new pipeline. **Decide:** still publish it? (It is the single biggest file and the historical Zenodo-upload-buster.)
2. **`annotations/archive/` (1264 files, 8.9 G)** — old archived annotations. Keep / drop?
3. **`products/json/` (current: 4 files, 113 M)** — `noctua-models-json.tgz`, `go-cam-cx2.tar.gz`, `go-cam-networkx.tar.gz`. **Circular dependency:** the new pipeline *consumes* `current.geneontology.org/products/json/noctua-models-json.tgz` as input (`MINERVA_JSON_TARBALL_URL`) but does not produce it. Decide producer + whether cx2/networkx are discontinued.
4. **Generated metadata derivatives** (current-only): `db-xrefs.json`/`.legacy`, `GO.xrf_abbs`, `groups.ttl`/`.tmp.jsonld`, `users.ttl`/`.tmp.jsonld`, `eco-usage-constraints.json`. The new pipeline rsyncs `go-site/metadata/` (the *source* YAML) but doesn't run the old yaml2json/yaml2turtle generation. Decide if any consumer needs the generated forms.
5. **`products/pages/downloads.html`** — the downloads page (already tracked); its input `combined.report.json` (409 M) currently lives only in the old `reports/`. Confirms the input-source problem already noted.
