...
(The former `s3://go-public/skyhook-geneontology-io/{union_*.gaf.gz,arbre.tgz}`
rows are **retired** — uploads removed 2026-08-14 after the GOlr indexer moved
to skyhook-HTTPS and a fleet fan-out confirmed no other consumers; audit
Exception 3 / operations#99. Stale objects may linger in the bucket.)
...
3. **`products/json/noctua-models-json.tgz`** — **DONE (#17).** Produced by
   `scripts/internal-all-gocam-products.sh` (Phase 5: `minerva-cli --dump-owl-json`
   → `tar` → `rsync` to `products/json/`, lines ~128/131/200). Its stage (**Internal
   all-GO-CAM products**, Jenkinsfile ~455) runs **before** the consumer (**GO-CAM
   processing** / `gocam-processing.sh` via `MINERVA_JSON_TARBALL_URL`, ~509), so the
   earlier "consumer-before-producer" ordering caveat **no longer applies**. Verified
   present on skyhook (HTTP 200) after run #85. (The `--dump-owl-json` reacto-neo NEO
   remote-pull is the accepted interim exception.) (`go-cam-cx2`/`networkx`:
   discontinued unless a consumer surfaces.)
