...
3. **`products/json/noctua-models-json.tgz`** — **MUST PRODUCE (confirmed not produced anywhere today).** pipeline-from-goa only *consumes* it (`MINERVA_JSON_TARBALL_URL`); pipeline-raw-go-cam has the tar+publish **commented out**; only the old `geneontology/pipeline` made it (`minerva-cli --dump-owl-json` → tar → `products/json/`). The dump mechanism already exists in `scripts/internal-all-gocam-products.sh`. **Ordering caveat:** the consumer (`gocam-processing.sh`) runs *before* the dump (`internal-all-gocam-products.sh`) in the current stage order, so producing it needs an earlier/dedicated stage or a reorder. (`go-cam-cx2`/`networkx`: treat as discontinued unless a consumer surfaces.)
4. **Generated metadata derivatives** (`db-xrefs.json`/`.legacy`, `GO.xrf_abbs`, `groups.ttl`, `users.ttl`, `eco-usage-constraints.json`) — **MUST ADD (confirmed).** `go-site/metadata/` ships only the *source* YAML (`groups.yaml`, `users.yaml`, `db-xrefs.yaml`); the derived forms are **absent** there and are generated by `go-site/scripts/` (`db-xrefs-yaml2legacy.js`, `yaml2turtle.sh`, …) in the old "Produce metadata" stage, which the new pipeline does not run (it only rsyncs the source YAML). Add the generation step; a quick consumer check first avoids emitting dead files.
5. **Downloads page** — **DESIGN SET: feed only `go-site/metadata/goex.yaml`.** Confirmed: goex.yaml (171 organisms; fields `code_uniprot`/`group`/`uniprot_proteome_id`/`taxon_id` + names) fully determines the file roster + labels + links, removing the `combined.report.json` dependency. **Caveat:** goex.yaml has **no** count/association field (verified across all 171 entries) → the page drops the per-dataset association counts the old page showed; `downloads-page-gen.py` needs a rewrite (it is currently hardwired to `combined.report.json` + a hardcoded species map).
