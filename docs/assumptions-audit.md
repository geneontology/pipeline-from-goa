...
| 3 | ~~union GAFs + PANTHER `arbre.tgz` re-hosted on go-public S3 (plain http)~~ | — | **RETIRED 2026-08-14** (number kept to preserve references): uploads removed after build #93 validated the skyhook-HTTPS path end-to-end and a fleet fan-out found zero other consumers (operations#99). History: the shim existed because of a **misdiagnosis** ("OWLTools can't read skyhook-HTTPS gzip" — really owltools#171 redirect-HTML + #329 https-scheme, both fixed in loader image `golr-autoindex:owltools-70527382_2026-08-10`; operations `docs/golr-index-build.md`). Rollback = revert the retirement commit + re-pin the prior image |
