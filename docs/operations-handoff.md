...
| **go-cam-browser data** | Bump the dated `release.geneontology.org/<YYYY-MM-DD>/products/go-cam-search/go-cam-browser-search-docs.json` URL in **`.env.production`** → merge to `main` → Pages auto-deploy. App fetches the product at runtime (go-cam-browser#74); the old `public/data.json` regenerate/commit is **retired**. CI `check:search-docs` gates the deploy on schema mismatch/404. | `go-cam-browser` | go-cam-browser#74; automation #72 |
